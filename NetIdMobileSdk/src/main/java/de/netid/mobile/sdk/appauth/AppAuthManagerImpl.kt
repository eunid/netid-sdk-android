...
internal class AppAuthManagerImpl(context: Context) : AppAuthManager {
    companion object {
        ...
        private const val SCOPE_PERMISSION_MANAGEMENT = "permission_management"
    ...
    override fun fetchAuthorizationServiceConfiguration(host: String) {
        ...
        AuthorizationServiceConfiguration.fetchFromIssuer(uriString.toUri()) { serviceConfiguration, authorizationException ->
        ...
        authorizationServiceConfiguration?.let { serviceConfiguration ->
            ...
            when (flow) {
                ...
                NetIdAuthFlow.LoginPermission -> {
                    scopes.add(AuthorizationRequest.Scope.OPENID)
                    scopes.add(SCOPE_PERMISSION_MANAGEMENT)
                }
     ...
     * @return NetIdError
     */
    private fun createNetIdErrorForAuthorizationException(
        process: NetIdErrorProcess,
